...
RUN bash -c "if [[ ! \"$VERSION\" > \"22.04\" ]]; then wget -O - https://apt.kitware.com/keys/kitware-archive-latest.asc 2>/dev/null | apt-key add -; fi"
...
RUN bash -c "if [[ ! \"$VERSION\" > \"22.04\" ]]; then \
